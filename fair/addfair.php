...
if(isset($_POST['txtRent'])){
    ...
    for($circle=0; $circle < $_POST['ddlTerm']; $circle++) {
      $sql = "INSERT INTO tbl_add_fair(type,floor_no,unit_no,payment_no,rid,month_id,xyear,rent,water_bill,electric_bill,gas_bill,security_bill,utility_bill,other_bill,total_rent,issue_date,term,agreement_no,paid_date,branch_id,bill_status) values('$type','$_POST[ddlFloorNo]','$_POST[ddlUnitNo]','$_POST[ddlPaymentMode]','$_POST[hdnRentedId]','$_POST[ddlMonth]','$_POST[ddlYear]','$_POST[txtRent]','$_POST[txtWaterBill]','$_POST[txtElectricBill]','$_POST[txtGasBill]','$_POST[txtSecurityBill]','$_POST[txtUtilityBill]','$_POST[txtOtherBill]','$_POST[txtTotalRent]','$_POST[txtIssueDate]','$_POST[ddlTerm]','$_POST[txtAgreementNo]','$_POST[txtPaidDate]','" . $_SESSION['objLogin']['branch_id'] . "','$_POST[ddlBillStatus]')";
      ...
      <form onSubmit="return validateMe();" action="<?php echo $form_url; ?>" method="post" enctype="multipart/form-data">
        <div class="box-body row">
          ...
          <div class="form-group col-md-12">
            <label for="ddlPaymentMode"><?php echo $_data['add_new_form_field_text_22'];?> :</label>
            <select name="ddlPaymentMode" id="ddlPaymentMode" class="form-control">
                <option value="">--<?php echo $_data['add_new_form_field_text_23']?>--</option>
                <?php
                  $result_payment = mysqli_query($link, "SELECT * FROM tbl_add_payment_setup order by p_id ASC");
                  while($row_payment = mysqli_fetch_array($result_payment))
                  {
                ?>
                    <option <?php if($payment_no == $row_payment['p_id']){echo 'selected';}?> value="<?php echo $row_payment['p_id'];?>"><?php echo $row_payment['payment_name']?></option>
                <?php
                  }
                ?>
            </select>
          ...
          <div class="form-group col-md-6">
            <label for="ddlTerm"><span class="errorStar">*</span> <?php echo $_data['add_new_form_field_text_20'];?> :</label>
            <select name="ddlTerm" id="ddlTerm" class="form-control">
              <option value="">--<?php echo $_data['add_new_form_field_text_20'];?>--</option>
              <?php 
                for($i=1; $i<=12; $i++) 
                {
              ?>
                  <option <?php if($select_term == $i){echo 'selected';}?> value="<?php echo $i;?>"><?php echo $i." ".$_data['add_new_rent_month'];?></option>
              <?php 
                } 
              ?>
            </select>
          ...
          <div class="form-group col-md-6">
            <label for="txtAgreementNo"><span class="errorStar">*</span> <?php echo $_data['add_new_form_field_text_21'];?> :</label>
            <input type="text" name="txtAgreementNo" id="txtAgreementNo" class="form-control" value="<?php echo $agreement_no?>"/>
          </div>
		      <div class="form-group col-md-6">
          ...
          <div class="form-group pull-right">
            ...
            <a class="btn btn-warning" href="<?php echo WEB_URL; ?>fair/fairlist.php"><i class="fa fa-reply"></i> <?php echo $_data['back_text'];?></a> 
          </div>
        </div>
...
function validateMe(){
    ...
    alert("<?php echo $_date['v44']; ?>");
    $("#ddlTerm").focus();
    return false;
  }
  else if($("#txtAgreementNo").val() == ''){
    alert("<?php echo $_date['v55']; ?>");
